...
BVH 模式约 \textbf{0.45\,s} 即可渲染出正确的着色图像。在此基础上，本文进一步新增逐三角形暴力求交路径，
并通过自动化实验对比三个不同面数的 Stanford 模型（Dragon、Armadillo）在 BVH 与无 BVH 下的渲染耗时，
BVH 加速比最高达 \textbf{8364} 倍。本文逐一说明各函数的算法原理、关键代码与实验结果。
\end{abstract}
...
\noindent 在 $1280\times960$ 分辨率下，Bunny 场景的 BVH 模式约 $0.45\,\mathrm{s}$ 即可完成渲染，
且两种模式输出\textbf{逐像素一致}，验证了求交与着色逻辑的正确性。
...
在四千余三角形的 Bunny 场景上以约 0.45\,s 渲染出正确结果。进一步的跨模型对照实验表明，相比逐三角形
暴力求交，BVH 在 $4.8$ 万至 $34.6$ 万面的 Stanford 模型上取得 $1368$ 至 $8364$ 倍加速，且渲染耗时几乎
不随面数增长（$0.39\to0.60\,\mathrm{s}$），凸显了层次包围盒在复杂场景下的必要性。通过本次实践，加深了对
光线生成、解析法求交、空间数据结构加速以及局部光照（Phong + 阴影）等图形学核心概念的理解。
